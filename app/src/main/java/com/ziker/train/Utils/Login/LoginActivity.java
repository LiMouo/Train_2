...
public class LoginActivity extends MyAppCompatActivity {
    private EditText le_username, le_password, re_user, re_pass1,le_pass2, re_telephone;
    ...
    private void InitView(){
        ...
        re_user = findViewById(R.id.le_user);
        re_pass1 = findViewById(R.id.le_pass1);
        le_pass2 = findViewById(R.id.le_pass2);
        re_telephone = findViewById(R.id.le_telephone);
   ...
   /*绑定数据*/
            ...
            if (Tools.isEmpty(e_name) || Tools.isEmpty(e_pass)){//账号或密码未输入
                ...
                myDialog.Do(v -> {
                    ...
                    v.findViewById(R.id.btn_ok).setOnClickListener(ok->{//确认点击事件
                        ...
                        re_user.setText(e_name);//注册账号设置为登录账号
                        v.dismiss();
                    });
    ...
    private void Register(){
        Cursor cursor = db.query("user", null, null, null, null, null, null);
        String res_name = re_user.getText().toString();
        String res_pass1 = re_pass1.getText().toString();
        String res_pass2 = le_pass2.getText().toString();
        String res_telephone = re_telephone.getText().toString();
        String num = "[1][345678]\\d{9}";//手机号正则规则
        if( Tools.isEmpty(res_name) || Tools.isEmpty(res_pass1) || Tools.isEmpty(res_pass2) || Tools.isEmpty(res_telephone)){
